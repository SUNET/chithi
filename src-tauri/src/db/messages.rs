...
/// Returns (message_id, maildir_path) pairs for each of the given IDs.
/// Rows with empty maildir_path are omitted.
...
mod tests {
    ...
    fn test_get_maildir_paths_returns_matching_rows() {
        ...
        let paths = get_maildir_paths(&conn, &ids).unwrap();
    ...
    fn test_get_maildir_paths_skips_empty_paths() {
        // Regression: cross-account move requires a non-empty maildir path so
        // the caller knows the message body has been synced. Rows with an
        // empty path (not-yet-fetched bodies) must be filtered out so the
    ...
    fn test_get_maildir_paths_empty_input() {
        let conn = setup_db();
        let paths = get_maildir_paths(&conn, &[]).unwrap();

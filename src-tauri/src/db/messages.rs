...
/// Backfill thread_ids for all messages in an account that currently have thread_id IS NULL.
/// Returns the count of messages updated.
/// Backfill thread_ids for all messages that don't have one yet.
///
/// Processes messages in date ASC order so that parent messages get their
/// thread_id set before replies, enabling In-Reply-To lookups to work.
/// Runs inside a single transaction for performance (26K+ messages take
/// seconds instead of minutes without a transaction).
///
/// This is a one-time migration — the `backfill_threads` command checks the
/// `app_metadata` table and skips if already done for this account.
pub fn backfill_thread_ids(conn: &Connection, account_id: &str) -> Result<u32> {
    log::info!(
        "backfill_thread_ids: starting backfill for account '{}'",
        account_id
    );

    // Get all messages with NULL thread_id, ordered by date ASC so earlier messages
    // get their thread_id set first and later replies can find them.
    let mut stmt = conn.prepare(
        "SELECT id, message_id, in_reply_to, subject FROM messages
    ...
    let total = rows.len();
    log::info!(
        "backfill_thread_ids: found {} messages to backfill",
        total
    );
    ...
    for (id, msg_id, in_reply_to, subject) in &rows {
    ...
    log::info!(
        "backfill_thread_ids: updated {}/{} messages for account '{}'",
        updated,
        total,
        account_id
    );
